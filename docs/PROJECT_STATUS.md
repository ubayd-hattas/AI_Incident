...
| A11 | Alex/Aaron/Jaswin | Freeze benchmark evidence, blind to policy outputs | ✅ **Done (Frozen)** | Full benchmark delivered and cryptographically frozen (`annotations/A11_BENCHMARK_SPEC.md`). 65 propositions (49 critical, 16 controls), 71 episodes (37 dev / 34 held-out), 25 copy groups, 1,361 occurrences across full 13,403 DSE corpus. 100% character spans/hashes verified by `audit/verify_annotations.py`. Split isolation verified (0 leakage). Ready for E12 evaluator. |
| E12 | Ubayd, Sam review | Evidence-coverage/delay evaluator | ⬜ **Not implemented/run; CONDITIONAL GO** | Semantic implementation unblocked on frozen A11 benchmark and schema. Substrate fixes/synthetic preparation are permitted; no real policy scoring until evaluator acceptance. |
...
**Plain read:** E08/E09/E10 implementation exists and the merged E09/E10 cross-policy audit records a collector-level PASS. Ticket A11 (Benchmark Evidence, Occurrences & Splits Freeze) is fully delivered and cryptographically verified: 65 propositions across 71 episodes in 25 title groups, zero split leakage, and 1,361 occurrences across DSE. Semantic evaluator E12 can now proceed against the frozen benchmark. All null, reversal and unmatched outcomes remain publishable.
