...
Living document. Latest checkpoint: **final hostile pre-results audit — CONDITIONAL GO E12; X13 NOT AUTHORIZED**. See `docs/PRE_RESULTS_AUDIT.md` and the frozen `docs/X13_RUN_CONTRACT.md`. No evidence coverage, policy-versus-label evaluation, or experimental results were run. Earlier dated entries are historical, not current authorization. Deadline recorded in the original plan: **2026-09-13, 23:59 AoE**.
...
**Track 2, "Evidence Before Erasure." E01/E05 PASS; V02/E06/V07 state-model acceptance retained with its documented limits. E08 and E09 P/PD/PCD and E10 E(q) are implemented; current offline suite: 103 tests PASS. Accounting arithmetic reproduces 9/9, but all 11 hashes in its prior acceptance record disagree with current files. Independent E09/cross-policy completion is reported by the user; its audit artifacts are absent locally. A03 pilot signoff is recorded, not an A11 benchmark freeze. E12 is not implemented/run; X13 is not run; no evidence-coverage result exists and the primary hypothesis is unknown.**
...
- Before E12: retrieve independent E09/E10 audit records with code hashes; reconcile/reaccept accounting pins; freeze A11 sample/groups/splits/criticality/fragments/context/encoding/eligibility; enforce retained-only snapshot and population boundaries.
...
- **Independent acceptance: DONE.** `audit/R04_ACCOUNTING_VERIFICATION.md` — Sam rebuilt the canonical serializer, packet/feed/header schemas, and FIFO/dedup/byte-hours logic from the contract text alone (not the README's arithmetic), ran it against all nine raw fixture files, and got 9/9 matches. File hashes of the amendment and all nine fixtures are pinned in that record.
- **E08 AUTHORIZED for observer/storage implementation.** Aaron's independent recheck remains open as an optional second confirmation (the amendment says "Sam or Aaron"), but is not a blocker per the amendment's own gate condition.
...
| E08 | Ubayd | Shared observer/storage | 🟡 **Implemented/neutrality-audited; pre-result fixes required** | Shared schemas/FIFO tests pass; audit found earlier-checkpoint response accounting, causal API guard and retained-only boundary gaps. See PRE_RESULTS audit A04/X01/X02. |
| E09–E10 | Ubayd | Periodic/event-derived collectors | 🟡 **P/PD/PCD and E(q) implemented; independent completion reported, records missing locally** | `src/ebe/collectors.py`, E09/E10 docs and tests. No E09 independent audit or `audit/E10_CROSS_POLICY_AUDIT.md` in checkout; supply before E12. F remains unimplemented, required before X13. No semantic results. |
| A11 | Alex/Aaron/Jaswin | Freeze benchmark evidence, blind to policy outputs | 🟡 **Independent annotation work proceeding; local final freeze absent** | A03 signed pilot exists. Need named sample/group graph/splits, full core/context occurrences, eligibility/encoding contracts and accepted hashes; gaps in PRE_RESULTS audit §8. |
| E12 | Ubayd, Sam review | Evidence-coverage/delay evaluator | ⬜ **Not implemented/run; CONDITIONAL GO only** | Semantic implementation waits for audit A01–A05 and A11 acceptance. Substrate fixes/synthetic preparation permitted; no real policy scoring. |
| X13 | Ubayd + Sam | Pinned sweep, clean-environment reproduction | ⬜ **Not run; NOT AUTHORIZED** | Run contract frozen; implementation/independent acceptance/mandatory sensitivities/reproduction gates still open. |
| P14 | Jaswin + Alex | Judge-facing artifact (figure + one history panel + README) | ⬜ Not started |

**Plain read:** collectors exist and no semantic outcome exists. The final audit finds a viable conditional comparison, but actual gate closure—not passing historical status prose—is required before E12/X13. All null/unmatched outcomes remain publishable.
...
- **Final pre-results methodological audit, baseline `9342cda`:** created `docs/PRE_RESULTS_AUDIT.md` and `docs/X13_RUN_CONTRACT.md`; verdict **CONDITIONAL GO E12**, X13 **NOT AUTHORIZED**. Frozen PCD15/E30 primary, unchanged10pp threshold, all-phase request admissibility, full matrix/statistics/outputs and mandatory archive/repair/order/chronology sensitivities without policy outcomes. Reproduced103 tests and9/9 accounting arithmetic; synthetic probes demonstrated checkpoint-accounting, causal API and unretained-body exposure issues. Found absent local E09/E10 independent reports and11/11 accounting acceptance hash mismatches. A03 pilot is not A11 acceptance. No collector/scorer/annotation code changed; no coverage or experiment run.
