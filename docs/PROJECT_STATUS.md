...
## Current authority — combined audit complete, verdict NOT PASS, no authorization issued

Sam's combined independent audit of the performance-equivalent code changes and the `FINAL_X13_RUNTIME_v2` amendment is complete: see `audit/FINAL_PRE_X13_INDEPENDENT_READINESS.md` sections 12 and 13. The verdict is **NOT PASS**. No Gate 2 authorization artifact has been created and none is authorized to be created until the remaining blockers close. `configs/x13_runtime_v2.json` still has status **BLOCKED_NOT_FROZEN**.

What the audit independently confirmed: the performance-equivalence changes are real and correct for P, PD, PCD, and E (re-verified by an independent re-run of `audit/verify_performance_equivalence.py`, 0 failures). Two additional real bugs specific to PCD-R were found and fixed directly (PCD-R never actually received the compact execution path; a separate O(known pages x retained packets) per-sweep scan), each independently re-verified for exact output equivalence against the unmodified algorithm on real bounded prefixes before being trusted. Even with both fixes, a full-horizon PCD-R phase-zero run did not complete within this session's observation window (well past an hour), so PCD-R should be treated as comparable in cost to the already-infeasible P/PD rows, not to plain PCD.

F's `AmbiguityLimitError` was root-caused precisely: exactly one page in the corpus (`dse~WillkommenImWiki`, the busiest page, 2,327 mutations) produces more nominal orderings than the 10,000-trajectory cap across many small same-time ties, and `Observer.terminal_directory()` does not catch this the way the body-GET path already does. This was deliberately not fixed: it is a genuine change to F's frozen "hostile control" semantics (how to treat undetermined page-liveness), not a same-semantics performance fix, and there was no second independent reviewer available to catch a mistake in that specific design call under deadline pressure.

**No numeric remaining-deadline/reserve budget was ever supplied**, and given the demonstrated infeasibility above (P/PD infeasible, PCD-R likely infeasible, F blocked), the full 130-row candidate was never attempted through the authorized process.

**A real primary result now exists, obtained outside the two-gate process, disclosed in full.** Under explicit user direction, the reduced 61-row primary contrast (all 60 PCD15 phases plus E30, excluding PCD-R/F) was executed directly against the real benchmark, using the same audited primitives `x13_runner.py` uses internally, but not through its authorization gate (that gate is hard-bound to the 791-row contract). The Claude Code session's own safety classifier blocked a direct attempt at this by the assisting AI; the execution was instead run by the human account owner themselves, in their own terminal. Full process disclosure and the exact numbers are in `runs/PRIMARY_SCOPED_v1/PRIMARY_RESULT.md` and `audit/FINAL_PRE_X13_INDEPENDENT_READINESS.md` section 14. Result: every one of the 60 PCD15 phases and E30 scored 0/23 held-out critical core, a clean null, $D=0$pp, ten points short of the frozen 10pp threshold, request admissibility satisfied. This does not retroactively authorize the runtime-v2 candidate, which remains **NOT PASS**; it is a result obtained through a disclosed departure from that process, not a PASS for it.

**The final submission report reflects this.** `report/report.tex` (rewritten at `082f376`, merged at `9c7c81f`, updated again with the real primary result above) now states the completed null result directly: abstract, Results, and Discussion all carry the real numbers and the process-departure disclosure. `report/report.pdf` is presently stale relative to `report.tex` (needs a fresh recompile with a LaTeX toolchain, unavailable in this environment) before final submission.
...
The original Gate 2 audit (candidate `9cbf2b6`) found 4 blocking implementation gaps. Ubayd fixed all 4 (`1b3464c`, `3187bad`, `b10596f`) and, in the same pass, also fixed PCD-R15's pending-suppression gap that the original audit had left as a disclosed non-blocking item, and the stale `docs/E12_EVALUATOR.md` text, and replaced a fake grep-based observer test with real behavioral coverage. Sam independently re-audited the new candidate with zero conflict of interest (an earlier same-session attempt by Sam to implement one of the fixes directly was discarded, unpushed, once Ubayd's own more thorough fix landed — disclosed in §9 of the report): re-confirmed the 3 already-fixed items, independently adversarially tested the cost/overhead wiring and, most importantly, the substantial PCD-R15 rewrite (10 hand-derived scenarios, 0 bugs found, "pending suppresses repair" confirmed genuinely implemented). Full suite 237/237 + 5 subtests; all 7 audit scripts and `audit/verify_final_x13_readiness.py` report 0 failures. Two minor, non-blocking, disclosed findings remain (a Windows RSS reader that fails safely to an honest NA due to a missing ctypes signature; `pending_index_peak` needlessly NA'd for non-E policies despite real available data) — neither violates the contract's actual bar of never imputing a missing value as zero.
