...
\vspace{0.5em}
\noindent Keywords: \todo{3--5 keywords, e.g. AI incident response,
...
\begin{itemize}[leftmargin=*]
  \item Track and problem. Track 2 (AI Incident Response). The
  ...
  \item Why it matters. This reframes ``AI incident response'' as
    ...
    sources}.
  \item Concrete motivating example (use this: it is real, already
  ...
  \item What the artifact is. (a) A frozen, hand-adjudicated
  ...
  \item Scope discipline (worth stating explicitly in the report: it is
    unusual and a strength, not a hedge). The primary contrast, the
...
\begin{itemize}[leftmargin=*]
  ...
  \item Gap and contribution (draft, tighten with citations): to our
...
\begin{itemize}[leftmargin=*]
  \item Trace-grounded simulation over a released wiki-style corpus (``DSE'')
    of autonomous-agent edits: 4{,}579 distinct pages and 14{,}591 total
    revisions (counted directly from \texttt{data/raw/export/revisions.jsonl})
    across a fixed replay window (\texttt{2026-05-24} to \texttt{2026-07-15}).
...
\begin{itemize}[leftmargin=*]
  ...
  \item Novelty/replication note worth including: the annotation
...
\begin{itemize}[leftmargin=*]
  \item Periodic: fixed-interval polling at several intervals
    $\Delta$ (P, PD, PCD variants), evaluated across all 60 phase offsets
    within each interval (not a single arbitrarily-chosen phase).
  \item Event-driven (E): feed-triggered collection, rate-limited by
    a token-bucket parameter $q$.
  \item Primary contrast (frozen before any result was seen): PCD at
  ...
  \item Hostile controls, run to test whether the primary contrast
    is a fair/robust comparison rather than an artifact of policy choice:
  \begin{itemize}
    \item \emph{F} is a low-information baseline: one terminal-state
      directory listing plus one atomic GET per currently-live title, no
      ongoing polling at all.
    \item \emph{PCD-R} is an online self-repair variant of periodic
    ...
    \item \emph{Terminal archive augmentation} is a full enumeration of every
...
\begin{itemize}[leftmargin=*]
  \item The evaluator scores \emph{only} what a policy's own capped,
    ...
    alternative).
  \item Causal-ordering guarantee (worth stating as a concrete,
...
\begin{itemize}[leftmargin=*]
  ...
  \item \pending{Cost/overhead comparison table: requests, bytes,
    byte-hours, and wall-clock/CPU/RSS at matched access, per the frozen
    accounting contract.}
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item Practical implication regardless of which way the result
...
\begin{itemize}[leftmargin=*]
  \item Simulated environment, not production telemetry. All agent
    ...
    systems.
  \item Benchmark scale and provenance. 65 hand-adjudicated
  ...
  \item Scope amendment. A originally-larger experimental matrix was
  ...
  \item No claim of real-world permanent erasure. A body missing
    ...
    it.
  \item No claim about agent intent. Wiki actor handles in the
  ...
  \item No general superiority claim. Any observed advantage for
...
\begin{itemize}[leftmargin=*]
  \item Defensive framing. This work is intended to inform
  ...
  \item Potential misuse vector. The same evidence-retention
  ...
  \item No exploit code released. \todo{confirm and state whether
